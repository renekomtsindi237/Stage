...
def _generer_dataset_synthetique(
    ...
    dates = pd.Series([
        date_debut + timedelta(days=int(d))
        for d in rng.integers(0, n_days, n)
    ])
...
def preparer_dataset_entrainement(fenetre_historique_jours: int = 730, **ctx) -> dict:
    ...
    if not X_all:
        logger.warning("Aucune donnée réelle — génération d'un dataset synthétique pour dev/test")
    ...
    logger.info(
        ...
        len(X_combined), taux_defaut, len(imf_ids),
    )
...
def entrainer_xgboost_mcrs(params: dict, composantes: dict, poids_composantes: dict, **ctx) -> dict:
    """
    ...
    dataset_path = Path(
        ...
        if ti else str(MODEL_BASE_DIR / "training_dataset.parquet")
    )
...
def valider_cross_validation(metriques: list[str], **ctx) -> dict:
    ...
    if auc < 0.65:
        logger.warning(
            "AUC challenger = %.4f < 0.65 — modèle de faible qualité, vérifier les features", auc
        )
...
def tracer_reliability_diagram(**ctx) -> dict:
    ...
    X    = challenger._preparer_features(df.drop(columns=["__label__", "__date__"], errors="ignore"))
    ...
    if len(y_cal) < 50 or y_cal.nunique() < 2:
        logger.warning("Ensemble de calibration trop petit ou mono-classe — diagramme ignoré")
    ...
    frac_pos_raw, mean_pred_raw = calibration_curve(y_cal, y_proba_raw, n_bins=10, strategy="uniform")
    frac_pos_cal, mean_pred_cal = calibration_curve(y_cal, y_proba_cal, n_bins=10, strategy="uniform")
    ...
    ax.plot(mean_pred_raw, frac_pos_raw, "b-o", label=f"XGBoost brut (Brier={brier_raw})")
    ax.plot(mean_pred_cal, frac_pos_cal, "r-o", label=f"XGBoost calibré Platt (Brier={brier_cal})")
    ...
    logger.info(
        ...
        output_path, brier_raw, brier_cal, amelioration,
    )
...
def ajuster_analyse_survie_cox(**ctx) -> dict:
    ...
    try:
        ...
        cox_features = ["jours_retard_moyen", "regularite_collecte_pct", "indice_resilience"]
...
def evaluer_shap_global(n_samples_shap: int = 1000, **ctx) -> dict:
    ...
    df = pd.read_parquet(dataset_path).drop(columns=["__label__", "__date__"], errors="ignore")
    sample = challenger._preparer_features(df.sample(min(n_samples_shap, len(df)), random_state=42))
...
def comparer_champion_challenger(
    ...
    **ctx,
    ...
    if row is None:
        ...
        _col_map = {
            "auc_roc": "auc_roc", "gini": "gini_coefficient",
            "gini_coefficient": "gini_coefficient", "ks_statistic": "ks_statistic",
            "brier_score": "brier_score", "f1_score": "f1_score",
        }
        champion_score = float(row[_col_map.get(metrique_comparaison, metrique_comparaison)] or 0)
        ...
        logger.info(
            ...
            metrique_comparaison, champion_score, challenger_score, delta,
            "PROMOUVOIR" if promouvoir else "GARDER_CHAMPION",
        )

    ti = ctx.get("ti")
    if ti:
        ti.xcom_push(key="challenger_metrics", value={"challenger_score": challenger_score})
    return promouvoir
...
def promouvoir_challenger(**ctx) -> str:
    ...
    with db_session() as cur:
        cur.execute("UPDATE ml.model_runs SET est_modele_actif = FALSE WHERE est_modele_actif = TRUE")
...
def enregistrer_run_mlflow(**ctx) -> int:
    ...
    shap_result = {}
    if ti:
        challenger_result = ti.xcom_pull(task_ids="entrainer_xgboost", key="challenger_metrics") or {}
        shap_result = ti.xcom_pull(task_ids="evaluer_interpretabilite", key="shap_global") or {}

    challenger = MCRSModel.charger(CHALLENGER_DIR)
    is_champion = CHAMPION_DIR.exists() and (
        (CHAMPION_DIR / "mcrs_model.pkl").stat().st_mtime ==
        (CHALLENGER_DIR / "mcrs_model.pkl").stat().st_mtime
        if (CHAMPION_DIR / "mcrs_model.pkl").exists() else False
    )
    ...
    params_dict = {
        k: v for k, v in vars(challenger.params).items()
    ...
    logger.info("model_run #%d enregistré (AUC=%.4f)", run_id, metrics.get("auc_roc", 0))
    return run_id
