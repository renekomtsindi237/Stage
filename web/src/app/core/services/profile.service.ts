...
import { UserPreferences, UserResponse, ChangePasswordRequest } from '../models/user.model';
...
export class ProfileService {
  ...
  getProfile(): Observable<UserResponse> {
    return this.http.get<ApiResponse<UserResponse>>(`${this.API}/me`).pipe(
      map(r => r.data)
    );
  }
  ...
  updatePreferences(prefs: Partial<UserPreferences>): Observable<UserResponse> {
    return this.http.patch<ApiResponse<UserResponse>>(`${this.API}/me/preferences`, prefs).pipe(
      map(r => r.data)
    );
  }
  ...
  changePassword(payload: ChangePasswordRequest): Observable<void> {
    return this.http.patch<ApiResponse<void>>(`${this.API}/me/password`, payload).pipe(
      map(() => void 0)
    );
  }
  ...
  uploadAvatar(file: File): Observable<UserResponse> {
    ...
    return this.http.post<ApiResponse<UserResponse>>(`${this.API}/me/avatar`, form).pipe(
      map(r => r.data)
    );
  }
  ...
  removeAvatar(): Observable<UserResponse> {
    return this.http.delete<ApiResponse<UserResponse>>(`${this.API}/me/avatar`).pipe(
      map(r => r.data)
    );
  }
}
