...
export class JwtInterceptor implements HttpInterceptor {
  ...
  intercept(req: HttpRequest<unknown>, next: HttpHandler): Observable<HttpEvent<unknown>> {
    ...
    const cleaned = skipRefresh
      ? req.clone({ withCredentials: true, headers: req.headers.delete('X-Skip-Auth-Refresh') })
      : req.clone({ withCredentials: true });

    return next.handle(cleaned).pipe(
      catchError((err: HttpErrorResponse) => {
        if (err.status === 401 && !skipRefresh && !req.url.includes('/api/auth/')) {
  ...
  private handle401(req: HttpRequest<unknown>, next: HttpHandler): Observable<HttpEvent<unknown>> {
    if (this.isRefreshing) {
      ...
      return this.refreshDone$.pipe(
        ...
        take(1),
        switchMap(v => {
          if (v === 'failed') return throwError(() => new Error('Session expirée'));
