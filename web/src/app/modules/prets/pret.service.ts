...
export class PretService {
  ...
  listPrets(
    ...
    page = 0,
    size = 20,
  ): Observable<PageResponse<PretResponse>> {
    let params = new HttpParams().set("page", page).set("size", size);
    if (statut) params = params.set("statut", statut);
    return this.http
      .get<ApiResponse<PageResponse<PretResponse>>>(this.API, { params })
      .pipe(map((r) => r.data));
  }

  getById(idPret: string): Observable<PretResponse> {
    return this.http
      .get<ApiResponse<PretResponse>>(`${this.API}/${idPret}`)
      .pipe(map((r) => r.data));
  }

  getPretsClient(idClient: string): Observable<PretResponse[]> {
    return this.http
      .get<ApiResponse<PretResponse[]>>(`${this.API}/client/${idClient}`)
      .pipe(map((r) => r.data));
  }
}
