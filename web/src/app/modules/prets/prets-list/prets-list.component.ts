...
export class PretsListComponent implements OnInit {
  ...
  readonly displayedColumns = ['idPret', 'nomClient', 'nomAgence', 'montantPret', 'joursRetard', 'statutPret', 'actions'];

  constructor(private pretService: PretService) {}

  ngOnInit(): void { this.loadPrets(); }

  loadPrets(): void {
    this.loading = true;
    this.error = '';
    this.pretService.listPrets(this.statutFiltre || undefined, this.page, this.pageSize).subscribe({
      next: (data) => {
  ...
  getStatutClass(statut: string): string {
    const map: Record<string, string> = {
      ACTIF: 'actif', EN_RETARD: 'en_retard',
      EN_RECOUVREMENT: 'en_retard', SOLDE: 'solde', PERTE: 'perte',
    };
    return map[statut] ?? '';
  }
  ...
  getStatutLabel(statut: string): string {
    const map: Record<string, string> = {
      ACTIF: 'Actif', EN_RETARD: 'En retard',
      EN_RECOUVREMENT: 'En recouvrement', SOLDE: 'Soldé', PERTE: 'Perte',
    };
