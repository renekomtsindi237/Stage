...
export class ProfileComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.pwdForm.get('newPassword')?.valueChanges.subscribe(() =>
  ...
  loadProfile(): void {
    ...
    this.profileService.getProfile().subscribe({
      ...
      error: () => { this.errorLoad = true; this.loading = false; },
    });
  }
  ...
  private uploadAvatarFile(file: File): void {
    ...
    reader.onload = ev => this.avatarPreview = (ev.target as FileReader).result as string;
  ...
  getStrength(): number {
    ...
    const score = [/[A-Z]/, /\d/, /[^A-Za-z0-9]/].filter(r => r.test(pwd)).length;
