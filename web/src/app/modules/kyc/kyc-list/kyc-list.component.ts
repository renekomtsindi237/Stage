...
import {
  KycDossierResponse, StatutKyc, NiveauKyc, NiveauRisque,
  TypeDocumentKyc, InitierKycRequest,
...
export class KycListComponent implements OnInit {
  ...
  readonly typePieceOptions: TypeDocumentKyc[] = [
    'CNI_RECTO', 'PASSEPORT', 'PERMIS_CONDUIRE', 'CARTE_SEJOUR'
  ];

  readonly displayedColumns = [
    'client', 'niveau', 'statut', 'risque', 'pep', 'expiration', 'actions'
  ];

  constructor(private kycService: KycService, private fb: UntypedFormBuilder) {}
  ...
  initierDossier(): void {
    ...
    this.kycService.initierDossier(req).subscribe({
      next: () => {
        ...
        this.initierForm.reset({ niveauDemande: 'NIVEAU_1', nationalite: 'Camerounaise', estPep: false });
        this.loadDossiers();
      },
  ...
  getNiveauLabel(n: NiveauKyc): string {
    return { NIVEAU_1: 'Niveau 1', NIVEAU_2: 'Niveau 2', NIVEAU_3: 'Niveau 3' }[n] ?? n;
  }
  ...
  getRisqueClass(r: NiveauRisque): string {
    const map: Record<NiveauRisque, string> = {
      FAIBLE: 'risk-faible', MOYEN: 'risk-moyen',
      ELEVE: 'risk-eleve', TRES_ELEVE: 'risk-tres-eleve'
    };
    return map[r] ?? '';
  }
  ...
  getPieceLabel(t?: string): string {
    const map: Record<string, string> = {
      CNI_RECTO: 'CNI', PASSEPORT: 'Passeport',
      PERMIS_CONDUIRE: 'Permis', CARTE_SEJOUR: 'Carte séjour',
    };
    return t ? (map[t] ?? t) : '';
  }
}
