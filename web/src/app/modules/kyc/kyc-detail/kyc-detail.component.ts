...
export class KycDetailComponent implements OnInit {
  ...
  readonly typeDocOptions: Array<{
    value: TypeDocumentKyc;
    label: string;
    niveau: number;
  }> = [
    ...
    {
      value: "EXTRAIT_BANCAIRE",
      label: "Extrait bancaire (3 mois)",
      niveau: 2,
    },
    {
      value: "DECLARATION_SOURCE_FONDS",
      label: "Déclaration source des fonds",
      niveau: 3,
    },
  ...
  loadAll(id: number): void {
    ...
    this.kycService.getDossier(id).subscribe({
      ...
      error: () => {
        this.error = "Dossier KYC introuvable.";
        this.loading = false;
      },
    });
  }
  ...
  validerDoc(doc: KycDocumentResponse, valide: boolean): void {
    const motif = valide
      ? undefined
      : (prompt("Motif de rejet du document :") ?? "");
  ...
  getNiveauLabel(n: NiveauKyc): string {
    return (
      {
        NIVEAU_1: "Niveau 1 — Simplifié",
        NIVEAU_2: "Niveau 2 — Standard",
        NIVEAU_3: "Niveau 3 — Renforcé",
      }[n] ?? n
    );
  }
  ...
  getDocIcon(t: TypeDocumentKyc): string {
    if (t.startsWith("CNI") || t === "PASSEPORT" || t === "PERMIS_CONDUIRE")
      return "badge";
    if (
      t.includes("DOMICILE") ||
    ...
    return (
      {
        APPROUVE: "res-ok",
        REJETE: "res-ko",
        COMPLEMENT_REQUIS: "res-complement",
      }[r] ?? ""
    );
  }
}
