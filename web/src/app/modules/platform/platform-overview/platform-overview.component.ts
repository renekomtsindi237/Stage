...
export class PlatformOverviewComponent implements OnInit {
  ...
  load(): void {
    ...
    this.platformService.getStats().subscribe({
      ...
      error: (err) => { this.errorCode = err?.status ?? -1; this.loading = false; }
    });
  ...
  private buildPaysStats(imfs: ImfRecord[]): void {
    ...
    imfs.forEach(i => { map[i.pays] = (map[i.pays] || 0) + 1; });
  ...
  get dsiCount(): number { return this.allImfs.filter(i => i.hasDsi).length; }

  /** Taux de couverture DSI */
  get dsiRate(): number {
    if (!this.allImfs.length) return 0;
    return Math.round((this.allImfs.filter(i => i.hasDsi).length / this.allImfs.length) * 100);
  }

  trackByImf(_: number, imf: ImfRecord): number { return imf.id; }
  trackByPays(_: number, p: { pays: string }): string { return p.pays; }

  getImfLogo(code: string): string {
    return this.auth.getImfLogo(code) || 'assets/photo_profil.jpg';
  }

  goToImf(): void { this.router.navigate(['/platform/imf']); }
  goToCreateImf(): void { this.router.navigate(['/platform/imf'], { queryParams: { action: 'create' } }); }
