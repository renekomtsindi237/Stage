...
import { PlatformService, ImfRecord, ImfSummary, AgenceSupervision, AuditEntry } from '../platform.service';
...
export class ImfSupervisionComponent implements OnInit {
  ...
  private loadHeader(): void {
    ...
    this.platformService.listImfs().subscribe({
      ...
      error: () => { this.errorHeader = true; this.loadingHeader = false; },
    });
  }
  ...
  loadUsers(page = 0): void {
    ...
    this.usersPage = page;
    this.platformService.getImfUsers(this.imfId, page, this.usersSize).subscribe({
      next: p => {
  ...
  usersNextPage(): void { if ((this.usersPage + 1) * this.usersSize < this.usersTotalElements) this.loadUsers(this.usersPage + 1); }
  usersPrevPage(): void { if (this.usersPage > 0) this.loadUsers(this.usersPage - 1); }
  ...
  loadAudit(page = 0): void {
    ...
    this.auditPage = page;
    this.platformService.getImfAudit(this.imfId, page, this.auditSize).subscribe({
      next: p => {
  ...
  auditNextPage(): void { if ((this.auditPage + 1) * this.auditSize < this.auditTotalElements) this.loadAudit(this.auditPage + 1); }
  auditPrevPage(): void { if (this.auditPage > 0) this.loadAudit(this.auditPage - 1); }

  // ── Helpers ───────────────────────────────────────────────────────────────

  back(): void { this.router.navigate(['/platform/imf']); }
  ...
  roleLabel(role: string): string {
    const labels: Record<string, string> = {
      DSI: 'DSI', DIRECTEUR: 'Directeur',
      RESPONSABLE_RECOUVREMENT: 'Resp. Recouvrement',
      ANALYSTE: 'Analyste', AGENT: 'Agent terrain',
    };
  ...
  get usersTotalPages(): number { return Math.ceil(this.usersTotalElements / this.usersSize); }
  get auditTotalPages(): number  { return Math.ceil(this.auditTotalElements  / this.auditSize); }
}
