...
import {
  DashboardDirecteur, ParStat, CollecteStat,
  TendancePrixProduit, BenchmarkAgence,
...
export class DashboardDirecteurComponent implements OnInit, OnDestroy {
    ...
    return [
      {
        ...
        value: this.summary.montantCollecteJour.toLocaleString('fr-FR') + ' FCFA',
        icon: 'savings',
      ...
      },
      {
        ...
        iconColor: this.objectifIconColor(this.summary.tauxRealisationObjectifPct),
      ...
      },
      {
        ...
        trend: this.summary.nbAlertesMlActives > 0
          ? `${this.summary.nbAlertesMlActives} alertes ML actives` : undefined,
        trendPositive: false,
      },
    ];
  }
  ...
  get parChartData() {
    ...
    return {
      ...
      datasets: [
        ...
        {
          label: 'Encours sain',
          data: this.parStats.map(s => Math.max(s.encoursTotal - s.encoursPar30, 0)),
  ...
  get collecteChartData() {
    const byDate = this.collecteStats.reduce((acc, s) => {
  ...
  ngOnInit(): void {
    ...
    this.sseSub = this.sseService.events$.subscribe(evt => {
      if (['kpi_collecte_updated', 'recouvrement_updated', 'scoring_updated'].includes(evt.type)) {
        this.charger();
      }
    });
  }
  ...
  private charger(): void {
    ...
    this.kpiService.getParStats(d30, today).subscribe(s => this.parStats = s);
    this.kpiService.getCollecteStats(d30, today).subscribe(s => this.collecteStats = s);
    this.kpiService.getBenchmarks().subscribe(b => this.benchmarks = b);
    this.kpiService.getTendancesPrix(undefined, undefined, 90).subscribe(t => this.tendancesPrix = t);
  }
