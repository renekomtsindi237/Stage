...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  get username(): string {
    return this.auth.getUsername() ?? "Utilisateur";
  }
  get imfNom(): string {
    return this.auth.getImfNom() ?? "Votre IMF";
  }

  get isDsi(): boolean {
    return this.role === "DSI";
  }
  get isDirecteur(): boolean {
    return this.role === "DIRECTEUR";
  }
  get isResponsable(): boolean {
    return this.role === "RESPONSABLE_RECOUVREMENT";
  }
  ...
  // ── Chart visibility ────────────────────────────────────────────
  get showParChart(): boolean {
    return true;
  }
  get showCanauxChart(): boolean {
    return this.isDirecteur || this.isAnalyste;
  }
  get showStatutChart(): boolean {
    return this.isDirecteur || this.isResponsable || this.isAnalyste;
  }
  get showAgencesChart(): boolean {
    return this.isDirecteur || this.isAnalyste;
  }
  get showEvolutionChart(): boolean {
    return !this.isAgent;
  }
  get showTasksSection(): boolean {
    return this.isDirecteur;
  }
  get showAgentsTable(): boolean {
    return this.isDirecteur || this.isResponsable;
  }
  ...
  get kpis(): DashboardKpi[] {
    ...
    if (this.isDsi)
      return [
        {
          label: "Encours total",
          value: this.fmtM(s.encoursTotal),
          icon: "account_balance",
          iconBg: "rgba(37,99,235,0.12)",
          iconColor: "#2563EB",
          footnote: "Portefeuille actif",
        },
        {
          label: "Alertes actives",
          value: String(s.nbAlertesActives),
          icon: "notifications_active",
          iconBg: "rgba(245,158,11,0.12)",
          iconColor: "#F59E0B",
          trend: s.nbAlertesActives > 0 ? "Attention requise" : "Aucune alerte",
          trendPositive: s.nbAlertesActives === 0,
        },
        {
          label: "Collectes MTD",
          value: this.fmtM(s.totalCollectes),
          icon: "payments",
        ...
        {
          label: "Prêts actifs",
          value: (s.nbCollectes || 0).toLocaleString("fr"),
          icon: "assignment",
          iconBg: "rgba(139,92,246,0.12)",
          iconColor: "#8B5CF6",
          footnote: "Dossiers en cours",
        },
      ];
    ...
    if (this.isDirecteur)
      return [
        {
          label: "PAR30",
          value: this.fmtM(s.encoursPar30),
          icon: "warning",
          iconBg: "rgba(239,68,68,0.12)",
          iconColor: "#EF4444",
          trend: "Risque portefeuille",
          trendPositive: false,
        },
        {
          label: "Collectes MTD",
    ...
    if (this.isResponsable) {
      const par30Rate =
        s.encoursTotal > 0
          ? ((s.encoursPar30 / s.encoursTotal) * 100).toFixed(1)
          : "0.0";
      return [
        {
          label: "PAR30",
          ...
          iconBg: "rgba(239,68,68,0.12)",
          iconColor: "#EF4444",
          trend: `${par30Rate}% du portefeuille`,
          trendPositive: false,
        },
        {
          label: "PAR90",
          value: this.fmtM(s.encoursPar90),
          icon: "error_outline",
          iconBg: "rgba(239,68,68,0.08)",
          iconColor: "#F87171",
          footnote: "Contentieux probable",
        },
        {
          label: "Alertes actives",
          value: String(s.nbAlertesActives),
      ...
      next: (event) => {
        if (event.type === "ALERTE_CREATED") {
          ...
          if (this.summary) {
            this.summary = {
              ...this.summary,
              nbAlertesActives: this.summary.nbAlertesActives + 1,
            };
          }
        }
      },
  ...
  get agencesTotalMCFA(): string {
    return this.agencesPerformanceData
  ...
  private prepareEvolutionMensuelleData(stats: ParStat[]): void {
    const monthlyData = new Map<
      string,
      { collectes: number; remboursements: number }
    >();
    stats.forEach((s) => {
    ...
    this.evolutionMensuelleLabels = sortedMonths.map((m) =>
      this.formatMonth(m),
    );
    this.evolutionMensuelleDatasets = [
      {
        ...
        data: sortedMonths.map(
          (m) => Math.round(monthlyData.get(m)!.collectes * 100) / 100,
        ),
        color: "#2563EB",
      },
      {
        ...
        data: sortedMonths.map(
          (m) => Math.round(monthlyData.get(m)!.remboursements * 100) / 100,
        ),
        color: "#10B981",
      },
    ];
