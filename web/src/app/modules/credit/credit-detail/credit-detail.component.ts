...
export class CreditDetailComponent implements OnInit {
  ...
  soumettre(): void {
    ...
    this.actionError = "";
    this.http
      .patch<any>(`/api/v1/dossiers-credit/${this.uid}/soumettre`, {})
      .subscribe({
        next: (r) => {
          this.dossier = r.data;
  ...
  validerChef(): void {
    ...
    const body = {
      approuve: this.validerApprouve,
      motif: this.validerMotif || undefined,
    };
    this.http
      .patch<any>(`/api/v1/dossiers-credit/${this.uid}/valider-chef`, body)
      .subscribe({
        next: (r) => {
          this.dossier = r.data;
          this.actionSuccess = this.validerApprouve
            ? "Dossier validé — envoyé en comité."
            : "Dossier rejeté.";
          this.actionLoading = false;
          if (this.dossier?.statut === "EN_COMITE") this.loadSeances();
        },
        error: (e) => {
          this.actionError =
            e?.error?.message ?? "Erreur lors de la validation.";
          this.actionLoading = false;
        },
      });
  }
  ...
  instructionComplete(): void {
    ...
    this.http
      .patch<any>(
        `/api/v1/dossiers-credit/${this.uid}/instruction-complete`,
        {},
      )
      .subscribe({
        next: (r) => {
  ...
  ouvrirSeance(): void {
    ...
    this.http
      .post<any>(`/api/v1/comite/dossier/${this.uid}/seance`, {})
      .subscribe({
        next: (r) => {
        ...
        error: (e) => {
          this.actionError =
            e?.error?.message ?? "Impossible d'ouvrir la séance.";
          this.savingSeance = false;
        },
      });
  }
  ...
  voter(): void {
    ...
    const body = {
      vote: this.voteValue,
      commentaire: this.voteCommentaire || undefined,
    };
    this.http
      .post<any>(`/api/v1/comite/dossier/${this.uid}/vote`, body)
      .subscribe({
        next: () => {
  ...
  enregistrerDecision(): void {
    ...
    this.http
      .post<any>(`/api/v1/comite/dossier/${this.uid}/decision`, body)
      .subscribe({
        next: (r) => {
        ...
        error: (e) => {
          this.actionError =
            e?.error?.message ?? "Erreur lors de l'enregistrement.";
          this.savingDecision = false;
        },
      });
  }
  ...
  get canSoumettre(): boolean {
    return (
      this.role === "AGENT_CREDIT" && this.dossier?.statut === "INSTRUCTION"
    );
  }

  get canValiderChef(): boolean {
    return (
      this.role === "CHEF_AGENCE" && this.dossier?.statut === "INSTRUCTION"
    );
  }

  get canInstructionComplete(): boolean {
    return (
      this.role === "ANALYSTE_ENGAGEMENTS" &&
      this.dossier?.statut === "EN_COMITE"
    );
  }

  get canOuvrirSeance(): boolean {
    return (
      ["CHEF_AGENCE", "DIRECTEUR", "DSI"].includes(this.role) &&
      this.dossier?.statut === "EN_COMITE"
    );
  }
  ...
  get canVoter(): boolean {
    return (
      ["CHEF_AGENCE", "DIRECTEUR", "ANALYSTE_ENGAGEMENTS"].includes(
        this.role,
      ) && this.dossier?.statut === "EN_COMITE"
    );
  }
  ...
  get canDecision(): boolean {
    return (
      ["CHEF_AGENCE", "DIRECTEUR"].includes(this.role) &&
      this.dossier?.statut === "EN_COMITE"
    );
  }
