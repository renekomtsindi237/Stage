...
export class SupAlertesComponent implements OnInit {
  ...
  readonly cols = ["timestamp", "type", "titre", "source", "severite", "statut", "actions"];
  ...
  acquitter(id: number): void {
    this.svc.acquitterAlerte(id).subscribe({
      next: () => { this.snack.open("Alerte acquittée", "Fermer", { duration: 2500 }); this.charger(); },
  ...
  getBadgeSeverite(s: string): string {
    return { CRITIQUE: "badge-critique", WARN: "badge-warn", INFO: "badge-info" }[s] ?? "";
  }

  getBadgeStatut(s: string): string {
    return { ACTIVE: "badge-critique", EN_COURS: "badge-warn", RESOLUE: "badge-ok" }[s] ?? "";
  }

  get nbCritiques(): number { return this.alertes.filter(a => a.severite === "CRITIQUE" && a.statut !== "RESOLUE").length; }
  get nbActives(): number { return this.alertes.filter(a => a.statut === "ACTIVE").length; }
}
