...
export class SupOverviewComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.poll$ = interval(20_000)
      .pipe(switchMap(() => this.svc.getSystemOverview()))
      .subscribe({
        next: (d) => {
          this.overview = d;
  ...
  getGlobalStatut(): "ok" | "warn" | "down" {
    ...
    if (this.overview.alertesCritiques > 0 || this.overview.containersDown > 0)
      return "down";
    if (this.overview.alertesActives > 0 || this.overview.containersDegrade > 0)
      return "warn";
    return "ok";
  }
}
