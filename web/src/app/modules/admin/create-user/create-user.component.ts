...
const DSI_ASSIGNABLE_ROLES: { value: Role; label: string; icon: string; description: string }[] = [
  {
    value: 'DIRECTEUR',
    label: 'Directeur',
...
export class CreateUserComponent implements OnInit {
  ...
  get step0Valid(): boolean {
    ...
    return (u?.valid && pw?.valid && cpw?.valid && !this.form.hasError('mismatch')
      && (emailCtrl?.valid ?? true)) ?? false;
  }
  ...
  requestGps(): void {
    ...
    navigator.geolocation.getCurrentPosition(
      pos => {
        ...
        this.longitude = Math.round(pos.coords.longitude * 1_000_000) / 1_000_000;
        this.updateMap();
  ...
  private updateMap(): void {
    ...
    const d   = 0.012;
    const raw = `https://www.openstreetmap.org/export/embed.html`
  ...
  onSubmit(): void {
    if (!this.step0Valid || this.form.get('role')?.invalid || this.loading) return;
    this.loading = true;
    this.error = '';
    ...
    this.adminService.createUser(payload).subscribe({
      ...
      error: (err) => {
        ...
        if (err?.status === 409) {
          ...
          this.error = 'Erreur lors de la création. Vérifiez les données saisies.';
        }
      }
    });
  }
  ...
  selectedRoleData() {
    return this.roles.find(r => r.value === this.form.get('role')?.value) ?? null;
  }
}
