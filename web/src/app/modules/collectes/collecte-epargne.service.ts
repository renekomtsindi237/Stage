...
export class CollecteEpargneService {
  ...
  soumettre(request: CollecteEpargneRequest): Observable<CollecteEpargne> {
    return this.http
      .post<ApiResponse<CollecteEpargne>>(this.API, request)
      .pipe(map((r) => r.data));
  }

  syncBatch(request: SyncCollectesRequest): Observable<SyncCollectesResponse> {
    return this.http
      .post<ApiResponse<SyncCollectesResponse>>(`${this.API}/sync`, request)
      .pipe(map((r) => r.data));
  }
  ...
  lister(
    ...
    return this.http
      .get<ApiResponse<PageResponse<CollecteEpargne>>>(this.API, { params })
      .pipe(map((r) => r.data));
  }
  ...
  kpiJour(date?: string): Observable<KpiJourAgent> {
    ...
    return this.http
      .get<ApiResponse<KpiJourAgent>>(`${this.API}/mon-kpi-jour`, { params })
      .pipe(map((r) => r.data));
  }

  collectesNonSynchros(): Observable<CollecteEpargne[]> {
    return this.http
      .get<ApiResponse<CollecteEpargne[]>>(`${this.API}/non-synchros`)
      .pipe(map((r) => r.data));
  }
}
