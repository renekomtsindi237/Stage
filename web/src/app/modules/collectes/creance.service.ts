...
export class CreanceService {
  ...
  lister(
    ...
    return this.http.get<ApiResponse<PageResponse<Creance>>>(this.API, { params })
      .pipe(map(r => r.data));
  }

  detail(id: number): Observable<Creance> {
    return this.http.get<ApiResponse<Creance>>(`${this.API}/${id}`)
      .pipe(map(r => r.data));
  }
  ...
  kpi(agenceId?: number, datePeriode?: string): Observable<KpiRecouvrement> {
    ...
    return this.http.get<ApiResponse<KpiRecouvrement>>(`${this.API}/kpi`, { params })
      .pipe(map(r => r.data));
  }

  scoreClient(clientId: string): Observable<ScoreMcrs> {
    return this.http.get<ApiResponse<ScoreMcrs>>(`${this.API}/client/${clientId}/score-mcrs`)
      .pipe(map(r => r.data));
  }

  majStatut(id: number, statut: string, observation?: string): Observable<Creance> {
    let params = new HttpParams().set('statut', statut);
    if (observation) params = params.set('observation', observation);
    return this.http.patch<ApiResponse<Creance>>(`${this.API}/${id}/statut`, null, { params })
      .pipe(map(r => r.data));
  }
}
