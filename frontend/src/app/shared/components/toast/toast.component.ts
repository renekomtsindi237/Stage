  ...
  animations: [
    ...
    trigger('cardPop', [
      transition(':enter', [
        ...
        animate('340ms cubic-bezier(0.34, 1.56, 0.64, 1)', style({ opacity: 1, transform: 'scale(1) translateY(0)' })),
      ]),
    ]),
    trigger('iconBounce', [
      transition(':enter', [
        style({ transform: 'scale(0.4)' }),
        animate('500ms 200ms cubic-bezier(0.34, 1.56, 0.64, 1)', style({ transform: 'scale(1)' })),
      ]),
    ]),
  ]
})
export class ToastComponent implements OnInit, OnDestroy {
  ...
  toast: ToastState = { type: 'success', title: '', message: '', show: false, duration: 2500 };
