...
@Component({
    ...
    @if (loading) {
      ...
      <div class="card stat-card">
        ...
        <div class="stat-card-value">
          @if (isCurrency) { {{ value | fcfa }} }
          @else if (isPercent) { {{ value | number:'1.1-1' }}% }
          @else { {{ value | number:'1.0-0' }} }
        </div>
        @if (trend !== undefined) {
          <div class="stat-card-trend" [class.up]="trendUp" [class.down]="!trendUp">
            <span class="material-icons-round">{{ trendUp ? 'trending_up' : 'trending_down' }}</span>
            {{ trend | number:'1.1-1' }}%
  ...
  styles: [`
    ...
    .stat-card-trend .material-icons-round { font-size: 14px; }
    ...
    .stat-card-loading { min-height: 90px; }
  `]
})
export class StatCardComponent {
  ...
  get trendUp(): boolean { return (this.trend ?? 0) >= 0; }
}
