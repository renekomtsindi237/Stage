...
interface CheckItem { libelle: string; conforme: boolean; echeance?: string; }
...
interface ConformiteData {
  ...
  engagements: { titre: string; montant: number; risque: string; dateEcheance: string; }[];
}
...
export class EngConformiteComponent implements OnInit {
  ...
  load() {
    ...
    this.api.get<ConformiteData>('/api/v1/engagements/conformite').subscribe({
      next: (d: ConformiteData) => { this.data.set(d); this.loading.set(false); this.cdr.markForCheck(); },
  ...
  risqueClass(r: string) {
    return { 'FAIBLE': 'badge-success', 'MOYEN': 'badge-warning', 'ELEVE': 'badge-danger', 'CRITIQUE': 'badge-danger' }[r] ?? '';
  }
