...
@Component({
  ...
  imports: [CommonModule, ReactiveFormsModule, RouterLink, FcfaPipe, AlertBadgeComponent],
...
export class DirClientsComponent implements OnInit {
  ...
  load() {
    ...
    this.api.get<ClientPage>('/api/v1/clients', {
      page: this.currentPage(), size: 20, search, statut, agence
    }).subscribe({
      next: (p: ClientPage) => { this.page.set(p); this.loading.set(false); },
