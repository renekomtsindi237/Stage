...
export class AcNouveauDossierComponent {
  ...
  prevStep() { this.step.update((s: number) => Math.max(1, s - 1)); }

  submit() {
    if (!this.selectedClient() || this.montantForm.invalid || this.garantieForm.invalid) return;
    ...
    this.api.post('/api/v1/dossiers-credit', payload).subscribe({
      next: () => {
        ...
        this.toast.showSuccess('Dossier créé', 'Le dossier de crédit a été enregistré.');
  ...
  get objectifLabel() {
    const m: Record<string, string> = {
      CREATION: 'Création d\'entreprise', FONDS_ROULEMENT: 'Fonds de roulement', INVESTISSEMENT: 'Investissement'
    };
