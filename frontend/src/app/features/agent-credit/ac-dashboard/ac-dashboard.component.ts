...
export class AcDashboardComponent implements OnInit {
  ...
  ngOnInit() {
    this.api.get<AcDashboard>('/api/v1/credit/dashboard').subscribe({
      next: (d: AcDashboard) => { this.data.set(d); this.loading.set(false); },
  ...
  statutClass(s: string): string {
    const m: Record<string, string> = {
      EN_INSTRUCTION: 'badge-moyenne', VALIDE_CHEF: 'badge-basse',
      APPROUVE: 'badge-basse', REJETE: 'badge-critique', SOUMIS: 'badge-haute'
    };
    return m[s] ?? '';
  }
}
