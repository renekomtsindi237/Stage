...
export class NouvelleCollecteComponent {
  ...
  submit() {
    ...
    this.api.post('/api/v1/collectes', payload).subscribe({
      next: () => { this.loading.set(false); this.saved.set(true); setTimeout(() => this.router.navigate(['/agent']), 1500); },
      error: () => { this.loading.set(false); this.error.set('Erreur lors de l\'enregistrement.'); }
    });
  }

  toggleGps() { this.gpsEnabled.update((v: boolean) => !v); }

  get typeOp() { return this.form.controls.typeOperation.value; }
}
