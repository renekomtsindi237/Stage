...
@Component({
  ...
  imports: [CommonModule, RouterLink, RouterLinkActive, FcfaPipe, AlertBadgeComponent],
...
export class AgentHomeComponent implements OnInit {
  ...
  load() {
    this.api.get<AgentDashboard>('/api/v1/agent/dashboard').subscribe({
      next: (d: AgentDashboard) => { this.data.set(d); this.loading.set(false); },
  ...
  sync() {
    ...
    setTimeout(() => { this.syncing.set(false); this.load(); }, 1500);
  }
