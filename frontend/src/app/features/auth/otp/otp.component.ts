import { Component, inject, signal, OnInit, OnDestroy, ChangeDetectionStrategy, ElementRef, ViewChildren, QueryList } from '@angular/core';
...
export class OtpComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.email = (nav?.extras?.state as { email?: string })?.email
  ...
  resend() {
    ...
    this.auth.requestOtp(this.email).subscribe({
      next: () => { this.countdown.set(60); this.startCountdown(); },
  ...
  private startCountdown() {
    ...
    this.timer = setInterval(() => {
      ...
      if (c <= 0) { clearInterval(this.timer); return; }
