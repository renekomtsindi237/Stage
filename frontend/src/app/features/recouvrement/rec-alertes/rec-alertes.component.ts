...
interface AlertePage { content: Alerte[]; totalElements: number; totalPages: number; number: number; }
...
export class RecAlertesComponent implements OnInit {
  ...
  load() {
    ...
    this.api.get<AlertePage>('/api/v1/recouvrement/alertes', { page: this.currentPage(), size: 20, statut }).subscribe({
      next: (p: AlertePage) => { this.page.set(p); this.loading.set(false); },
  ...
  prendre(id: number) {
    this.api.patch(`/api/v1/recouvrement/alertes/${id}/traiter`, {}).subscribe({
      next: () => { this.toast.showSuccess('En traitement', 'Alerte prise en charge.'); this.load(); },
      error: () => this.toast.showError('Erreur', 'Action impossible.')
    });
  }

  resoudre(id: number) {
    this.api.patch(`/api/v1/recouvrement/alertes/${id}/resoudre`, {}).subscribe({
      next: () => { this.toast.showSuccess('Résolue', 'Alerte marquée résolue.'); this.load(); },
      error: () => this.toast.showError('Erreur', 'Action impossible.')
    });
  }
  ...
  niveauClass(n: string) {
    return { 'AMIABLE': 'badge-warning', 'FORMEL': 'badge-primary', 'JUDICIAIRE': 'badge-danger' }[n] ?? '';
  }
  statutClass(s: string) {
    return { 'OUVERTE': 'badge-danger', 'EN_TRAITEMENT': 'badge-warning', 'RESOLUE': 'badge-success' }[s] ?? '';
  }
}
