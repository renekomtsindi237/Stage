...
export class AnlScoringComponent implements OnInit {
  ...
  load() {
    ...
    this.api.get<ScoringPage>('/api/v1/analyste/scoring', {
      page: this.currentPage(), size: 15, classe, search
    }).subscribe({
      next: (p: ScoringPage) => { this.page.set(p); this.loading.set(false); },
  ...
  classeLabel(c: string) {
    const map: Record<string, string> = {
      TRES_FAIBLE: 'Très faible', FAIBLE: 'Faible',
      MOYEN: 'Moyen', BON: 'Bon', TRES_BON: 'Très bon'
    };
    return map[c] ?? c;
  }
  ...
  classeClass(c: string) {
    const map: Record<string, string> = {
      TRES_FAIBLE: 'badge-critique', FAIBLE: 'badge-haute',
      MOYEN: 'badge-moyenne', BON: 'badge-basse', TRES_BON: 'badge-success'
    };
    return map[c] ?? '';
  }
}
