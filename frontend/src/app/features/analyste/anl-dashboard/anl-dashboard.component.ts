...
@Component({
  ...
  imports: [CommonModule, BaseChartDirective, StatCardComponent, AlertBadgeComponent],
...
export class AnlDashboardComponent implements OnInit {
  ...
  barChartData: ChartData<'bar'> = {
    ...
    datasets: [{ data: [], label: 'Clients', backgroundColor: [
      '#ef4444', '#f59e0b', '#3b82f6', '#22c55e', '#1b2f4b'
    ]}]
  };
  ...
  ngOnInit() {
    this.api.get<AnlDashboardData>('/api/v1/analyste/dashboard').subscribe({
      next: (d: AnlDashboardData) => {
        ...
        this.barChartData = {
          ...this.barChartData,
          datasets: [{ ...this.barChartData.datasets[0], data: d.scoringDistribution.map(s => s.count) }]
        };
