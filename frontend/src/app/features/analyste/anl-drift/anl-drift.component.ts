...
export class AnlDriftComponent implements OnInit {
  ...
  psiChartOptions: ChartConfiguration<'line'>['options'] = {
    ...
    scales: {
      y: { min: 0, ticks: { callback: v => String(v) }, grid: { color: '#f1f5f9' } },
  ...
  ngOnInit() {
    this.api.get<MlDrift>('/api/v1/analyste/ml/drift').subscribe({
      next: (d: MlDrift) => { this.data.set(d); this.buildChart(d); this.loading.set(false); },
  ...
  get psiClass(): string {
    ...
    if (psi > 0.20) return 'high';
    if (psi > 0.10) return 'medium';
    return 'ok';
  }

  maxContrib(): number {
    return Math.max(...(this.data()?.contributionFeatures ?? []).map((f: { nom: string; contribution: number }) => f.contribution), 1);
  }
}
