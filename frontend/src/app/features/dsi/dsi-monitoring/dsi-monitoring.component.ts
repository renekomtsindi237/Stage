...
export class DsiMonitoringComponent implements OnInit {
  ...
  load() {
    ...
    this.api.get<ImfHealth>('/api/v1/dsi/monitoring/health').subscribe({
      next: (h: ImfHealth) => { this.imfHealth.set(h); this.loadingHealth.set(false); this.cdr.markForCheck(); },
      error: () => { this.loadingHealth.set(false); this.cdr.markForCheck(); }
    });

    this.api.get<RecentLogin[]>('/api/v1/dsi/monitoring/connexions-recentes').subscribe({
      next: (list: RecentLogin[]) => { this.recentLogins.set(list); this.loadingLogins.set(false); this.cdr.markForCheck(); },
      error: () => { this.loadingLogins.set(false); this.cdr.markForCheck(); }
    });
  }

  statutClass(s: string) {
    return { 'OK': 'badge-success', 'AVERTISSEMENT': 'badge-warning', 'CRITIQUE': 'badge-danger' }[s] ?? '';
  }
