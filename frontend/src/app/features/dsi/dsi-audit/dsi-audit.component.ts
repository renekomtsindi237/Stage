...
interface AuditPage { content: AuditEntry[]; totalElements: number; totalPages: number; number: number; }
...
export class DsiAuditComponent implements OnInit {
  ...
  load() {
    ...
    this.api.get<AuditPage>('/api/v1/admin/audit/trail', {
      page: this.currentPage(), size: 20, action, utilisateur
    }).subscribe({
      next: (p: AuditPage) => { this.page.set(p); this.loading.set(false); },
