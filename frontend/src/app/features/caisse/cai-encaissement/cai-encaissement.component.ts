...
export class CaiEncaissementComponent {
  ...
  search(q: string) {
    ...
    this.api.get<{ content: Client[] }>('/api/v1/clients', { search: q, size: 5 })
      .subscribe((r: { content: Client[] }) => this.searchResults.set(r.content));
  }

  select(c: Client) { this.selectedClient.set(c); this.searchResults.set([]); }

  submit() {
    if (!this.selectedClient() || this.form.invalid) return;
    this.loading.set(true);
    this.api.post('/api/v1/caisse/encaissements', {
      ...
      next: () => {
        ...
        this.toast.showSuccess('Encaissement enregistré', `${this.form.value.montant?.toLocaleString('fr-FR')} FCFA reçus.`);
        this.router.navigate(['/caisse/dashboard']);
      },
      error: () => { this.loading.set(false); this.toast.showError('Erreur', 'Impossible d\'enregistrer l\'encaissement.'); }
    });
  }
}
