...
export class SupMonitoringComponent implements OnInit {
  ...
  loadHealth() {
    ...
    this.api.get<ActuatorHealth>('/actuator/health').subscribe({
        ...
        const rows: ServiceHealth[] = [
          ...
          ...(h.components
            ? Object.entries(h.components).map(([k, v]) => ({
                name: k, status: (v.status === 'UP' ? 'UP' : 'DEGRADED') as ServiceHealth['status']
              }))
            : [])
        ];
      ...
      error: () => {
        this.services.set([{ name: 'Backend API', status: 'DOWN', details: 'Inaccessible' }]);
        this.loading.set(false);
        this.cdr.markForCheck();
      }
    });
  ...
  statusClass(s: string) {
    return { 'badge-success': s === 'UP', 'badge-warning': s === 'DEGRADED', 'badge-danger': s === 'DOWN' };
  }
}
