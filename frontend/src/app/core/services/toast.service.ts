...
export class ToastService {
  private state$ = new BehaviorSubject<ToastState>({
    type: 'success', title: '', message: '', show: false, duration: 2500
  });
  ...
  showLogin(username: string, isSuperAdmin = false, duration = 2500) {
    ...
    this.state$.next({ type: 'login', title: 'Connexion réussie !', message, show: true, username, duration });
  ...
  private emit(type: ToastState['type'], title: string, message: string, duration: number) {
