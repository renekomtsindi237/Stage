...
interface ApiPage { content: NotificationItem[] }
...
export class NotificationService implements OnDestroy {
  ...
  init() {
    ...
    this.sseSub = this.sse.connect().subscribe(event => {
      ...
      const item: NotificationItem = {
        id: 0, type: event.type,
        titre: this.titleForType(event.type),
        message: event.message,
        lu: false, createdAt: event.timestamp
      };
  ...
  loadInitial() {
    ...
    this.http.get<ApiResponse<ApiPage>>('/api/v1/notifications?page=0&size=30')
  ...
  markAsRead(id: number) {
    if (id === 0) return;
    this.http.put<ApiResponse<void>>(`/api/v1/notifications/${id}/read`, {}).subscribe({
      next: () => {
        this.items$.next(this.items$.value.map(n => n.id === id ? { ...n, lu: true } : n));
      },
      error: () => {}
    });
  }
  ...
  markAllAsRead() {
    this.http.put<ApiResponse<void>>('/api/v1/notifications/read-all', {}).subscribe({
      next: () => {
