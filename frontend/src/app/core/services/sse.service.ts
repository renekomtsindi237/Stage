...
export class SseService implements OnDestroy {
  ...
  private readonly EVENT_TYPES = [
    'HEARTBEAT', 'ALERTE_CREATED', 'ALERTE_UPDATED',
    'COLLECTE_CONFIRMED', 'KPI_UPDATED', 'PIPELINE_STATUS', 'SYNC_COMPLETED'
  ];
  ...
  connect(): Observable<SseEvent> {
    ...
    this.eventSource = new EventSource('/api/v1/sse/stream', { withCredentials: true });
    ...
    this.eventSource.onerror = () => {
      ...
      if (this.reconnectAttempts >= this.MAX_RECONNECT) {
        ...
        this.http.get('/api/v1/users/me', { withCredentials: true }).subscribe({
          ...
          error: () => { /* auth interceptor handles logout */ }
        });
        return;
      }
